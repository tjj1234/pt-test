...
归因看板目前是**骨架版（演示级）**，不是生产级 BI 工具：顶部筛选栏、5 个 Tab 导航、数据 API 对接、真库 SQL 都已打通，但各 Tab 实现完整度不一。5 个 Tab 与状态：

| Tab | 状态 | 说明 |
|-----|------|------|
| 总览 Overview | ✅ 已接通 | KPI 卡 + 转化漏斗 + 日趋势（`GET /api/analytics/funnel`） |
| 素材 Creatives | ✅ 已接通 | 素材级 ROI 排行（`roi_by_entity`） |
| 用户 Users | 🟡 部分 | 注册→建Key→充值转化路径已接通；**用户行为详情抽屉是占位容器，未接真实数据** |
| 产品 Product | 🟡 部分 | 模型调用 / 渠道 / 充值趋势已接通；**Token 消耗卡是空态**（funnel 契约未返回 token 字段，不编造、不写 0） |
| 健康 Health | ✅ 已接通 | 打点健康度评分 + Token 审计 report（无数据时显式「暂不可用」） |

**已知未实现/占位**（不回避）：
1. 素材 Tab 的「广告侧指标」（消耗 / 点击 / CTR / 缩略图）—— 待广告侧明细接口。
2. 用户 Tab 的「用户行为详情抽屉」—— 仅占位容器。
3. 产品 Tab 的「Token 消耗」—— funnel 契约缺口，走空态兜底。
4. 看板前端为原生 JS/HTML，无构建、无 E2E 测试；只有后端 SQL 自检与语法检查（见 `.github/workflows/ci.yml`）。
