...
async function run(ctx, task, io) {
	...
	// STREAM: 把 assistant 文本增量（text-delta）实时写到 stdout，逐字透传
	let streamed = false;
	ctx.on("session/event", (session, event) => {
		...
		if (chunk.type === "text-delta" && chunk.text) {
			streamed = true;
			try { writeSync(1, chunk.text); } catch (e) { io.stdout.write(chunk.text); }
		}
	});
	...
	if (!streamed) io.stdout.write(outcome.text);
	io.stdout.write("\n");
